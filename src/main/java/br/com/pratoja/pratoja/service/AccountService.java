...
public class AccountService {
    ...
    @Transactional public String requestCustomerReset(String email){Optional<User> user=users.findByEmailIgnoreCase(email==null?"":email);if(user.isEmpty()||user.get().getRole()!=DomainTypes.Role.CUSTOMER)return null;String raw=UUID.randomUUID()+""+UUID.randomUUID();PasswordResetToken t=new PasswordResetToken();t.setUser(user.get());t.setTokenHash(hash(raw));t.setExpiresAt(LocalDateTime.now().plusMinutes(30));tokens.save(t);return raw;}
