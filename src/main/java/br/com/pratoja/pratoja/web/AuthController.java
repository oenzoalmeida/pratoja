...
public class AuthController {
    private final AccountService accounts;@Value("${pratoja.demo-mode:true}")private boolean demo;@Value("${pratoja.demo-reset-email:cliente@pratoja.com.br}")private String demoResetEmail;
    ...
    @PostMapping("/recuperar-senha")String forgot(@RequestParam String email,Model model){model.addAttribute("success","Se o e-mail estiver cadastrado, um link de recuperação foi gerado.");String token=null;if(demo){if(email!=null&&email.strip().equalsIgnoreCase(demoResetEmail))token=accounts.requestCustomerReset(email.strip());}else token=accounts.requestReset(email);if(demo&&token!=null)model.addAttribute("demoLink","/redefinir-senha?token="+token);return "auth/forgot";}
